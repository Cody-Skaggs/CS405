// CS405.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if ((increment > 0 && result > std::numeric_limits<T>::max() - increment) || (increment < 0 && result < std::numeric_limits<T>::min() - increment)) {
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if ((decrement < 0 && result >= std::numeric_limits<T>::max() - decrement) || (decrement > 0 && result <= std::numeric_limits<T>::min() + decrement)) {
